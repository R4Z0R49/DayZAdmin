...
function markers_player($res, $world) {
	$markers = array();
		foreach($res as $row) {
			...
			$description = "<strong><a href=\"admin.php?view=info&show=1&id=".$row['unique_id']."&cid=".$row['cid']."\">".htmlspecialchars($row['name'], ENT_QUOTES)." - ".$row['unique_id']."</a></strong> <table> <tr> <td><img style=\"width: 100px;\" src=\"images/models/".str_replace('"', '', $row['model']).".png\"></td> <td>&nbsp;</td> <td style=\"vertical-align:top; \"> <strong>PlayerID:</strong> ".$row['id']."<br> <strong>CharacterID:</strong> ".$row['unique_id']."<br> <strong>Zed Kills:</strong> ".$row['zombie_kills']."<br> <strong>Bandit Kills:</strong> ".$row['bandit_kills']."<br> <strong>Alive Duration:</strong> ".survivalTimeToString($row['survival_time'])."<br><strong>Position:</strong>&nbsp;".sprintf("%03d%03d", round(world_x($x, $world)), round(world_y($y, $world)))."<br><strong>Humanity:</strong>&nbsp;".$row['humanity']."</td></tr></table>";	
			$tmp = array();
