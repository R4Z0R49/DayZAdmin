...
function markers_player($res, $world) {
	...
	global $row_playerName;
	global $row_Worldspace;
	global $row_CharacterID;
	global $row_PlayerID;
	global $row_PlayerUID;
	global $row_Alive;
	global $row_Model;
	global $row_KillsZ;
	global $row_KillsB;
	...
	global $row_Humanity;
		...
		foreach($res as $row) {	
			...
			$tmp = array();
			$tmp["id"] = $row[$row_CharacterID];
			$tmp["lat"] = (world_y($y, $world) / 10);
			$tmp["lng"] = (world_x($x, $world) / 10);
			$tmp["icon"] = "Player".($row[$row_Alive] == 'is_dead' ? $row[$row_Alive] : !$row[$row_Alive]);
...
function markers_vehicle($res, $world) {
	...
	global $row_ObjectWorldspace;
	global $row_ObjectClassname;
	global $row_ObjectType;
	global $row_ObjectID;
	global $row_ObjectUID;
	global $row_Objectlast_updated;
	global $row_ObjectDamage;
	global $row_ObjectFuel;
